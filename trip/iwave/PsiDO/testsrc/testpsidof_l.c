...
int main(int argc, char **argv) {
  ...
  FQ = (float complex *)  malloc(nx*nz*sizeof(float complex) );printf("2 \n");
  ...
  */
  ...
  for (m=0;m<nx;m++)
      ...
      for (n=0;n<nz;n++)
	{
	  ...
	  */
	  ...
	  FQ[(m*nz+n)]=z*cpow(exp(1),I*2*x)/4.0;//cexp causing trouble use cpow instead!
  ...
  double elapsed;
  start = clock();
  PsiDOf_l(U,FQ,QU,nx,nz,dx,dz,m_ord,l);
  end=clock();
  ...
  for( m=0;m<nx;m++)
    {
      for(n=0;n<nz;n++)
	{
	  printf("%f ",creal(QU[m*nz+n]));printf("+I %f",cimag(QU[m*nz+n]));printf("   ");
	}
      printf("\n");
    }
  printf("Check result against Matlab Benchmark code! \n");
