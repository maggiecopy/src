...
//#define VERBOSE 1
//tests the Qsquare code by trying to squre cos(x+theta) to cos(x+theta)^2
//both have simple fourier expansions and may be checked directly
//the error in squaring should only be due to DFT which is usually machine
//epsiolon * norm(vector)
//Also tests the symmetrizing code, the error should be zero since all is done
//in exact arithmetic
