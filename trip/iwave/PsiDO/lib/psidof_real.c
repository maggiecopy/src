...
void PsiDOf_real(fftwf_complex const  *U ,fftwf_complex const *FQ,fftwf_complex *QU ,int nx, int nz, int k, double dx,double dz,double m_ord){
	  ...
	  for(n=0;n<nz;n++)
	    {
	      if ( l>=0)
		{
		  // printf("setting QU, l= %d \n",l);
		  QU[m*nz+n]=QU[m*nz+n]+ FQ[l/2+k*(m*nz+n)]*R[m*nz+n]/(nx*nz);
		}
	      else
		{// printf("setting QU, l= %d \n",l);
		  
		  QU[m*nz+n]=QU[m*nz+n]+ conj(FQ[-l/2+k*(m*nz+n)])*R[m*nz+n]/(nx*nz);
		}
