	subroutine cubicadj(x1i,hi,vi,ni,x1o,ho,vo,no,iend,work,wl,ierr)
...
c
	if (iend.lt.1.or.iend.gt.3) then
		ierr=11
		write(6,*)'** Error in cubic adjoint program **'
		write(6,*)'   incorrect end condition given'
		write(6,*)' choose end condition between 1 and 3'
		return
	end if
...
c
	if (no.lt.4) then
		ierr=12
		write(6,*)'** Error in cubic adjoint program **'
		write(6,*)'   not enough output data points'
		return
	end if
	if (ni.lt.3) then
		ierr=12
		write(6,*)'** Error in cubic adjoint program **'
		write(6,*)'   not enough input data points'
		return
	end if
...
c
	if (wl.lt.ni+7*no-8) then
		ierr=10   
		write(6,*)'**Error in cubic adjoint program**'
		write(6,*)'      not enough workspace'
		write(6,*)
		write(6,*)wl,' was length provided'
		write(6,*)ni+7*no-8,'was length needed'
		return
	end if 
