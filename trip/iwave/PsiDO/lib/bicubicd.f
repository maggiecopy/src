	subroutine bicubicd(x1in,dxin,nxin,y1in,dyin,nyin,zin,
...
c
	...
	do 100 i=1,nxin
		...
		work(j)=zin(i,j)
 50		continue
	call svaluesd(y1in,dyin,work(ptr1),nyin,work(ptr2),iend,
     1	             work(ptr3),gw,ierr)
		do 60 j=1,nyin
...
c
 ...
 150	continue
	...
	do 200 j=1,nxin
     ...
     &          work(j+(int-1)*nxin+g1+g2+gw))/(6D0*dyin)
           b=work(j+g1+g2+gw+(int-1)*nxin)/2D0
           c=((zin(j,int+1)-zin(j,int))/dyin)-
     &          ((2D0*dyin*work(j+g1+g2+gw+(int-1)*nxin)+dyin*
     &          work(j+int*nxin+g1+g2+gw))/6D0)
...
c
	call cubicd(x1in,dxin,work(ptr1),nxin,x1out,dxout,work(ptr2),     
