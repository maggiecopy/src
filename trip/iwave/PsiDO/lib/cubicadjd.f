	subroutine cubicadjd(x1i,hi,vi,ni,x1o,
     &    ho,vo,no,iend,work,wl,ierr)

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     Cubic Spline Adjoint
c     by Ann Campbell
c
c     function:  This program generates the adjoint to the cubic
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	integer trial
	double precision x1i,hi,vi(ni),x1o,ho,vo(no),work(wl),u,dx
...
c
c	now check if an appropriate end condition has been provided
c
	if (iend.lt.1.or.iend.gt.3) then
		ierr=11
c		write(6,*)'** Error in cubic adjoint program **'
...
c
c	check if there are enough input and output data points for
c	the program to work properly
c
	if (no.lt.4) then
		ierr=12
c		write(6,*)'** Error in cubic adjoint program **'
...
c       program.	
c	*note:  to work correctly, this program requires the input
c	vector to have length at least 3 and the output vector to
c	have length at least 4
c
...
c
 ...
 20	continue
	...
	do 30 i=no-1,2,-1
	work(i+ni)=work(i+ni-1)/(6D0*ho)-work(i+ni)/(6D0*ho)
	work(i+ni+no)=work(i+ni+no)/2D0
	work(i+ni+2*no)=(ho/6D0)*work(ni+2*no+i-1)+
     1    (2D0*ho)/6D0*work(ni+2*no+i)
 30	continue

	work(ni+1)=work(ni+1)*(-1D0/(6D0*ho))
	work(ni+no)=temp1/(6D0*ho)
	work(ni+no+1)=work(ni+no+1)/2D0
	work(ni+2*no)=0.0D0
	work(ni+2*no+1)=(2D0*ho)/6D0*work(ni+2*no+1)
	work(ni+3*no)=(ho/6D0)*temp2
...
c
	...
	if (iend.eq.3) then
		work(ni+2)=work(ni+2)+2D0*work(ni+1)
		work(ni+3)=work(ni+3)-work(ni+1)
		work(ni+no-2)=work(ni+no-2)-work(ni+no)
		work(ni+no-1)=work(ni+no-1)+2D0*work(ni+no)
		work(ni+no+2)=work(ni+no+2)+2D0*work(ni+no+1)
		work(ni+no+3)=work(ni+no+3)-work(ni+no+1)
		work(ni+2*no-2)=work(ni+2*no-2)-work(ni+2*no)
		work(ni+2*no-1)=work(ni+2*no-1)+2D0*work(ni+2*no)
	 	work(ni+2*no+2)=work(ni+2*no+2)+2D0*work(ni+2*no+1)
		work(ni+2*no+3)=work(ni+2*no+3)-work(ni+2*no+1)
		work(ni+3*no-2)=work(ni+3*no-2)-work(ni+3*no)
		work(ni+3*no-1)=work(ni+3*no-1)+2D0*work(ni+3*no)
	end if
...
c
	...
	if (iend.eq.2) then
		...
		work(ni+4*no-1)=6D0*ho
		work(ni+3*no+2)=0.0D0
		work(ni+6*no-7)=0D0
		work(ni+5*no-4)=6D0*ho
	end if
...
c
	...
	do 110 i=no-2,3,-1
	work(ni+i)=6D0/ho*work(ni+i-2)-12D0/ho*work(ni+i-1)+
     1	6D0/ho*work(ni+i)
	work(ni+no+i)=6D0/ho*work(ni+no+i-2)
     1    -12D0/ho*work(ni+no+i-1)
     1    +6D0/ho*work(ni+no+i)
	work(ni+2*no+i)=6D0/ho*work(ni+2*no+i-2)
     1    -12D0/ho*work(ni+2*no+i-1)
     1    +6D0/ho*work(ni+2*no+i)
 110	continue



	work(ni+2)=-12D0/ho*work(ni+1)+6D0/ho*work(ni+2)
	work(ni+1)=6D0/ho*work(ni+1)
	work(ni+no-1)=6D0/ho*temp6-12D0/ho*temp3
	work(ni+no)=6D0/ho*temp3
	work(ni+no+2)=-12D0/ho*work(ni+no+1)+6D0/ho*work(ni+no+2)
	work(ni+no+1)=6D0/ho*work(ni+no+1)
	work(ni+2*no-1)=6D0/ho*temp7-12D0/ho*temp4
	work(ni+2*no)=6D0/ho*temp4
	work(ni+2*no+2)=-12D0/ho*work(ni+2*no+1)+6D0/ho*work(ni+2*no+2)
	work(ni+2*no+1)=6D0/ho*work(ni+2*no+1)
	work(ni+3*no-1)=6D0/ho*temp8-12D0/ho*temp5
	work(ni+3*no)=6D0/ho*temp5
...
c
 ...
 130	continue
	work(ni+4*no+1)=-1D0/ho*work(ni+4*no+1)
...
c
	...
	vo(i)=work(ni+3*no+i)+work(ni+i)+work(ni+no+i)-work(ni+2*no+i)
     1		+ work(ni+4*no+i)
c
c     the next multiplication translates the transpose to being
c     the adjoint
c
