...
//    Vector<ireal> ng(op.getDomain());
//    Vector<ireal> g(op.getDomain());
//    Vector<ireal> Aimg(op.getRange());
//    AssignFilename Fmfn(valparse<std::string>(*pars,"FTFimg"));
//    g.eval(Fmfn);
//    
//    cop.applyAdjOp(dd,ng); 
//    cerr << "ng.norm()=" << ng.norm() << endl;
//    cop.applyOp(ng,Aimg);
//    cerr << "Aimg.norm()=" << Aimg.norm() << endl;
//    cop.applyAdjOp(Aimg,g);
//    cerr << "g.norm()=" << g.norm() << endl;
//    psido.applyOp(ng,g);
//

    PsiCGNEAlg<float> alg(dm,cop,psido, dd,
